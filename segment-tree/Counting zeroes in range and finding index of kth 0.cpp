...
int query(int v, int stl, int str, int l, int r)
{
    ...
    int stm = (stl + str) / 2;
    return query(v * 2, stl, stm, l, min(r, stm)) + query(v * 2 + 1, stm + 1, str, max(l, stm + 1), r);
}
...
int kth(int v, int stl, int str, int k)
{
    //deb(v);
    if (k > st[v])
        return -1;
    ...
    if (st[v * 2] >= k)
    {
        return kth(v * 2, stl, stm, k);
    }
    else
    {
        return kth(v * 2 + 1, stm + 1, str, k - st[v * 2]);
    }
}
int32_t main()
{
