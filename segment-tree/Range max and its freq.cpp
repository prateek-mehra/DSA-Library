...
pair<int,int> st[4 * N];
int a[N];
pair<int,int>combine(pi a,pi b)
{
    if(a.first>b.first)
    return a;
    if(b.first>a.first)
...
pair<int,int> query(int v, int stl, int str, int l, int r)
{
    //deb(v);
    //deb2(stl,str);
    if (l > r)
    ...
    int stm = (stl + str) / 2;
    return combine(query(v * 2, stl, stm, l, min(r, stm)),query(v * 2 + 1, stm + 1, str, max(l, stm + 1), r));
}
